...
public class GroupVote {
    ...
    public GroupVote(ApplicationContext applicationContext) {
        state = new State(applicationContext);
    }

    public GroupVote(ApplicationContext applicationContext, JSONObject jsonObject) {
    ...
    public static class State {
        ...
        private boolean operationVote(Operation operation) {
            ...
            for (Object object: voteArray) {
                ...
                applicationContext.logDebugMessage("POLL: " + poll.toJSONObject().toJSONString());
            }
            ...
            if (poll != null && poll.size() != 0 ) {
                ...
                for (Poll item: list) {
                    ...
                    if (item.removeAfter >= 0 && (item.heightEnd + item.removeAfter < currentHeight)) {
                        applicationContext.logInfoMessage("GroupVote: expired poll: " + item.id);
                        poll.remove(item.id);
                    }
                }
            }
